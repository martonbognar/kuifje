...
basicI n =
  ...
  while3 (\s -> return (s^.ans && s^.i < n))                        -- |while (ans && i<N) do|
    (                                                               -- |begin|
    cond3 (\s -> return ((s^.pw !! s^.i) /= (s^.gs !! s^.i)))       -- \quad |if (pw[i] /= gs[i])|
          (update3 (\s -> return (s.^ans $= False)))                -- \qquad |then ans := false|
...
basicL n =
  update3 (\s -> return (s.^i $= 0)) <--->                          -- |i := 0;|
  update3 (\s -> return (s.^ans $= True)) <--->                     -- |ans := true|
  while3 (\s -> return (s^.i < n))                                  -- |while i<N do|
    (                                                               -- |begin|
    cond3 (\s -> return ((s^.pw !! s^.i) /= (s^.gs !! s^.i)))       -- \quad |if (pw[i] /= gs[i])|
          (update3 (\s -> return (s.^ans $= False)))                -- \qquad |then ans := false|
          skip3 <--->                                               -- \qquad |else skip|
...
basicM n =
  ...
  while3 (\s -> return (s^.i < n))                                  -- |while i<N do|
    (                                                               -- |begin|
    (update3 (\s -> return (s.^ans $=                               -- \quad |ans :=|
...
basicR n =
  ...
  update3 (\s -> return (s.^ans $= True)) <--->                     -- |ans := true;|
  while3 (\s -> return (s^.ans && not (null (s^.l))))               -- |while (ans && l/=[]) do|
    (                                                               -- |begin|
    update3 (\s -> uniform [s.^i $= j | j <- s^.l]) <--->           -- \quad |i := uniform(l);|
    (update3 (\s -> return (s.^ans $=                               -- \quad |ans :=|
      (s^.ans && (s^.pw !! s^.i) == (s^.gs !! s^.i))))) <--->       -- \qquad |ans && (pw[i] = gs[i]);|
